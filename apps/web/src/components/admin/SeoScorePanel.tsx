  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 8, padding: "6px 0", borderBottom: "1px solid #f3f4f6" }}>
      <span style={{
        ...
        backgroundColor: STATUS_COLORS[check.status], flexShrink: 0, marginTop: 4,
      }} />
      <div style={{ flex: 1 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: "#374151" }}>{check.label}</div>
        <div style={{ fontSize: 12, color: "#6b7280", marginTop: 2 }}>{check.message}</div>
      </div>
    </div>
  );
}
...
export function SeoScorePanel() {
  ...
  return (
    <div style={{ border: "1px solid #e5e7eb", borderRadius: 8, padding: 16, backgroundColor: "#fafafa", marginTop: 8 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 20, marginBottom: 16 }}>
        <ScoreCircle score={analysis.overallScore} />
        <div>
          <div style={{ fontSize: 16, fontWeight: 700, color: "#111827" }}>SEO Puanı</div>
          <div style={{ display: "flex", gap: 12, marginTop: 8, fontSize: 13 }}>
            <span style={{ color: STATUS_COLORS.good }}>● {analysis.summary.good} İyi</span>
            <span style={{ color: STATUS_COLORS.improvement }}>● {analysis.summary.improvement} Geliştir</span>
            <span style={{ color: STATUS_COLORS.poor }}>● {analysis.summary.poor} Zayıf</span>
          </div>
        </div>
      </div>
      <div>
        {analysis.checks
          ...
          .map((check) => <CheckItem key={check.id} check={check} />)}
      </div>
    </div>
  );
}
